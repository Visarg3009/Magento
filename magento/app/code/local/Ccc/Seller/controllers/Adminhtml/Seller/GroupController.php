...
class Ccc_Seller_Adminhtml_Seller_GroupController extends Mage_Adminhtml_Controller_Action
{
    public function saveAction()
    {
        ...
        if ($model->itemExists()) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('seller')->__('A group with the same name already exists.'));
        } else {
            try {
                $model->save();
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('seller')->__('An error occurred while saving this group.'));
            }
        }
    }
